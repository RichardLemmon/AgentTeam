...
describe('resolveExpansionRequest', () => {
  it('approves a request', () => {
    ...
    const result = resolveExpansionRequest(db, { request_id: (req as any).id, status: 'approved', resolution_note: 'Good call' });
  ...
  it('denies a request', () => {
    const db = setupDb();
    const req = requestTeamExpansion(db, { project_id: 'p1', requested_by: 'm1', role_needed: 'data_engineer', justification: 'Need help' });
    const result = resolveExpansionRequest(db, { request_id: (req as any).id, status: 'denied', resolution_note: 'Covered by existing scope' });
