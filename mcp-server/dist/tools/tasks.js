...
export function createTask(db, input) {
    ...
    if (input.assignee_id) {
        ...
        if (!member)
            return notFound('Team member', input.assignee_id);
    }
    const id = crypto.randomUUID();
    const now = new Date().toISOString();
    db.prepare('INSERT INTO tasks (id, project_id, assignee_id, title, description, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(id, input.project_id, input.assignee_id ?? null, input.title, input.description, 'pending', now, now);
...
export function updateTask(db, input) {
    ...
    if (!task)
        return notFound('Task', input.task_id);
    ...
    db.prepare('UPDATE tasks SET status = ?, description = ?, assignee_id = ?, updated_at = ? WHERE id = ?').run(status, description, assigneeId, now, input.task_id);
    return { id: input.task_id, status, assignee_id: assigneeId, updated_at: now };
}
export function getTask(db, input) {
    const err = validateRequired(input.task_id, 'task_id');
    if (err)
        return err;
    const row = db.prepare('SELECT * FROM tasks WHERE id = ?').get(input.task_id);
    if (!row)
        return notFound('Task', input.task_id);
