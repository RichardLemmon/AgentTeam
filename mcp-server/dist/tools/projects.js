...
export function deleteProject(db, input) {
    const idErr = validateRequired(input.project_id, 'project_id');
    if (idErr)
        return idErr;
    ...
    const deleteInTransaction = db.transaction(() => {
        ...
        const taskIds = db.prepare('SELECT id FROM tasks WHERE project_id = ?').all(input.project_id).map(r => r.id);
        const discIds = db.prepare('SELECT id FROM discussions WHERE project_id = ?').all(input.project_id).map(r => r.id);
